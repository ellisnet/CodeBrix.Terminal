...
namespace CodeBrix.Terminal.Engine.CommandExtensions; //was previously: namespace XtermSharp.CommandExtensions;
...
internal static class TerminalBufferManipulationCommandExtensions {
    ...
    /// </summary>
    // TODO: Investigate off-by-one in pars.Length checks — each guard uses `pars.Length > N`
    //  where N is one too high for the 0-based index being accessed (e.g. `pars.Length > 8`
    //  to read `pars[7]` should be `pars.Length > 7`). This affects all eight parameter
    //  extractions and causes the target-page parameter to silently default to 1.
